...
def process_melt_accum_thresh(melt_thresh_df,df_sum_total,elev_bin,isAccum):
    """
    Slice data based on accumulation / melt threshold for each year.
    Input:
      melt_thresh_df - pandas dataframe of melt threshold values.
...
def regression_statistics(y_true,
                          y_pred,
                          features,
                          date,
                          modelID,
    ...
    percent_resid = 100*((y_true - y_pred ) / y_true)
    swe_msk = y_true >= mask_val
    resid = y_true - y_pred


    limit_vals_resid = resid[swe_msk]
    limit_vals_resid = limit_vals_resid[~np.isnan(limit_vals_resid)]
    limit_vals_percent = percent_resid[swe_msk]
    limit_vals_percent = limit_vals_percent[~np.isnan(limit_vals_percent)]
    try: 
        max_resid_absolute = np.abs(limit_vals_resid).max() 
    except: 
        max_resid_absolute = np.nan
    try: 
        max_resid_percent = np.abs(limit_vals_percent).max() 
    except:
        max_resid_percent = np.nan
...
def run_mlr_train_predict(aso_tseries_1,
                          obs_data_hist,
                          elev_band,
                          all_pils,
                          all_pils_QA,
    ...
    Input:
      ...
      impute_df - dataframe for imputation values.
      melt_thresh_df - dataframe for melt threshold values.
    ...
    if isSplit:
        if isImpute:
            ...
            if isMean:
                ...
                obs_data_5_,pils_removed,df_summary_impute = impute_pillow_prediction(df_sum_total,
                                                                                      all_pils_QA,
                                                                                      obs_data_hist,
                                                                                      aso_site_name,
                                                                                      prediction_date,
            ...
            df_split = process_melt_accum_thresh(melt_thresh_df,
                                                 df_summary_impute,
                                                 elev_band,
                                                 isAccum
                                                 )
            predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names, aso_tseries_2, obs_data_6 = run_cross_val_selection(obs_data_5_,df_split,aso_tseries_1,pils_removed,start_wy,end_wy,
            ...
            predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names, aso_tseries_2, obs_data_6 = run_cross_val_selection(obs_data_hist,df_split,aso_tseries_1,baseline_pils,start_wy,end_wy,
                                                                                                                         elev_band,isCombination = isCombination,showOutput = showOutput,isMelt = isSplit)
    else:
        if isImpute:
            ## update missing observations with average from flight date.
            ...
            predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names, aso_tseries_2, obs_data_6 = run_cross_val_selection(obs_data_5_,df_summary_impute,aso_tseries_1,pils_removed,start_wy,end_wy,
                                                                                                                         elev_band,isCombination = isCombination,showOutput = showOutput,isMelt = isSplit)
        else:
            predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names, aso_tseries_2, obs_data_6 = run_cross_val_selection(obs_data_hist,df_sum_total,aso_tseries_1,baseline_pils,start_wy,end_wy,
                                                                                                                         elev_band,isCombination = isCombination,showOutput = showOutput,isMelt = isSplit)
            

    summary_dict = regression_statistics(aso_tseries_2[:,elev_band].values * mmScalar, 
                                      predictions_validation * mmScalar,
    ...
    return predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names,aso_tseries_2, obs_data_6, summary_dict, df_final
...
def run_cross_val_selection(obs_data,df_sum_total,aso_tseries,all_pillows,start_wy,end_wy,
    ...
    predictions_bestfit = summarize(summary_data_total)
    if showOutput:
    ...
    selected_station_ids = identify_best_stations(best_stations,aso_tseries_2,elev_band,summary_data_total,isCombination,showOutput = showOutput)
    predictions_bestfit = cross_val_loo(aso_tseries_2[:,elev_band], summary_data_total[selected_station_ids,:])
    predictions_bestfit = np.array(predictions_bestfit)
    predictions_bestfit[predictions_bestfit<0]=0

    selected_station_names = [obs_data_6[i].name for i in range(0,len(obs_data_6)) if i in selected_station_ids]
    ...
    return predictions_bestfit, predictions_validation, selected_station_ids, selected_station_names,aso_tseries_2, obs_data_6


def _to_scalar(x, name="value") -> float:
    """Convert x (scalar/0d/1d length-1) to float, else raise with shape info."""
    if x is None:
    ...
    if arr.ndim == 0:
        return float(arr)
    if arr.size == 1:
        return float(arr.reshape(()))  # or float(arr.item())
    raise ValueError(f"{name} is not scalar: shape={arr.shape}, type={type(x)}")

def run_mlr_inference(df_final_train: pd.DataFrame,
                      df_current_vals: pd.DataFrame,
                      dem_bin_da: xr.DataArray,
                      elev_idx: int,
                      selected_pils: list,
                      summary_dict: dict,
                      add_zeroASO: bool = True,
                      ModelID: int = 0,
):
    """
    Run multiple linear regression inference given selected pillows.
    Input:
      df_final_train - DataFrame of final training data.
      df_current_vals - DataFrame of current values for inference.
      dem_bin_da - xarray DataArray of DEM bins.
      elev_band - integer of elevation bin.
      selected_pils - list of selected pillows.
      conversion - conversion factor.
      area_m2 - area in square meters.
      add_zeroASO - boolean to indicate adding zero ASO.
    Output:
      predictions_inference - numpy array of predictions.
    """

    elev_vals = dem_bin_da[elev_idx].values.flatten()
    # count of valid grids.
    count = len(elev_vals[~np.isnan(elev_vals)])
    # count to m2.
    area_m2 = count * 50 * 50
    # predictions in mm
    yhat_mm,df_train,yhat_test = run_daily_prediction(df_final_train,
                                                      df_current_vals.reset_index(names='time'),
                                                      selected_pils,
                                                      conversion = 0,
                                                      area_m2 = area_m2,
                                                      add_zeroASO = add_zeroASO)
    # predictions in acre ft
    yhat_acreft,df_train,yhat_test = run_daily_prediction(df_final_train,
                                                      df_current_vals.reset_index(names='time'),
                                                      selected_pils,
                                                      conversion = 2,
                                                      area_m2 = area_m2,
    ...
    summary_dict[ModelID]['prediction']['mm'].append(_to_scalar(yhat_mm,"yhat_mm"))
    ## prediction acre-ft
    summary_dict[ModelID]['prediction']['acre_ft'].append(_to_scalar(yhat_acreft,"yhat_acreft"))
...
def run_daily_prediction(df_train,
                         df_test,
                         selected_pils,
                         conversion = 0,
                         area_m2 = None,
